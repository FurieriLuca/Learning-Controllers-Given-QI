...
convexprogram_outputfeedback;  %% solves the program with convex programming, so that we know the optimal cost and solution
...
eta_dividers=[1000;200;50;25;10;10;7.5];   %%empirically found for good convergence at every considered precision
r_dividers=[1;1;1;1;1;1.2;1.2];   %%empirically found for good convergence at every considered precisions
for(precisions = 1:size(epsilons,2))
    ...
    for(rounds = 1:rounds_max)
        ...
        if(eval_cost(parameters)-optimal_cost<eps)
            ...
            avg_final_cost(precisions) = (avg_final_cost(precisions) + final_cost)/rounds_max;
    ...
    fprintf('%d/10 attempts were succesful for precision eps = %f . \n',count_succeeded(precisions),epsilons(precisions));
