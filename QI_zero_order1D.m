...
QI_convexprogram1D;  %% solves the program with convex programming, so that we know the optimal cost and solution
optimal_cost = value(cost); %%%optimal cost

%ORACLE SETUP
create_system;  %%creates the dynamical system,
...
parameters = -1%[1.38+1;-1.6;1.40];%10*(rand(cardinality,1)-rand(cardinality,1));
...
for(precisions = 1:size(epsilons,2))
    ...
    eta = 1*0.000100*eps^2; %115% %% stepsize (goes with eps^2)
